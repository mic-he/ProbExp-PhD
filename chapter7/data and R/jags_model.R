...
model{  
  ...
  beta_ax=(1-omega_ax)*(kappa_ax-2)+1
  
  theta.might~dunif(theta.min, theta.max) # these are the semantic thresholds
  theta.probably~dunif(theta.min, theta.max)
  theta.certainly~dunif(theta.min, theta.max)
  theta.possible~dunif(theta.min, theta.max)
  theta.likely~dunif(theta.min, theta.max)
  
  # priors on parameters, all uniform with various supports
